...
// Structure organisée : niveau 1 (subcategories) et niveau 2 (subcategories_level2)
const structure = {
    // Géographie
    'Géographie': {
    ...
    // Politique (garder les existantes)
    'Politique': {
...
async function main() {
    try {
        console.log('🚀 Réorganisation des sous-catégories Actualités en 2 niveaux\n');
        
        // 1. Récupérer la catégorie Actualités
        console.log('📋 Récupération de la catégorie Actualités...');
        ...
        // 2. Configurer la catégorie pour avoir le niveau 2
        console.log('⚙️  Configuration du niveau 2...');
        await supabase
        ...
        console.log('✅ Configuration niveau 2 activée\n');
        
        // 3. Supprimer toutes les sous-catégories existantes (sauf Breaking news, Analyses politiques, International)
        console.log('🗑️  Suppression des anciennes sous-catégories...');
        const { data: existing } = await supabase
            .from('subcategories')
            .select('id, name')
            .eq('category_id', categoryId);
        
        const toKeep = ['Breaking news', 'Analyses politiques', 'International'];
        const toDelete = existing?.filter(s => !toKeep.includes(s.name)) || [];
        
        if (toDelete.length > 0) {
            // Supprimer d'abord les niveau 2 associés
            const subcategoryIds = toDelete.map(s => s.id);
            await supabase
                .from('subcategories_level2')
                .delete()
                .in('subcategory_id', subcategoryIds);
            
            // Puis supprimer les sous-catégories
            await supabase
                .from('subcategories')
                .delete()
                .in('id', subcategoryIds);
            
            console.log(`✅ ${toDelete.length} sous-catégorie(s) supprimée(s)\n`);
        } else {
            console.log('ℹ️  Aucune sous-catégorie à supprimer\n');
        }
        
        // 4. Créer les nouvelles sous-catégories niveau 1 et niveau 2
        ...
        const { data: finalLevel2 } = await supabase
            .from('subcategories_level2')
            .select('id')
            .in('subcategory_id', finalLevel1?.map(s => s.id) || []);
