...
async function main() {
    try {
        console.log('🔍 Vérification de la catégorie Makeup\n');
        
        // Chercher la catégorie (peut être "Makeup", "Maquillage", etc.)
        const { data: category } = await supabase
            .from('categories')
            .select('id, name, color, description')
            .or('name.ilike.%makeup%,name.ilike.%maquillage%')
            .single();
        
        if (!category) {
            console.log('❌ Catégorie "Makeup" introuvable\n');
