-- Script pour compléter les tables de contenu existantes (version corrigée)
...
-- 2. Ajouter les contraintes de clés étrangères (sans IF NOT EXISTS)
DO $$ 
BEGIN
    -- content_blogs
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_blogs_author') THEN
        ALTER TABLE content_blogs ADD CONSTRAINT fk_content_blogs_author FOREIGN KEY (author_id) REFERENCES auth.users(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_blogs_category') THEN
        ALTER TABLE content_blogs ADD CONSTRAINT fk_content_blogs_category FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_blogs_subcategory') THEN
        ALTER TABLE content_blogs ADD CONSTRAINT fk_content_blogs_subcategory FOREIGN KEY (subcategory_id) REFERENCES subcategories(id) ON DELETE SET NULL;
    END IF;
    
    -- content_articles
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_articles_author') THEN
        ALTER TABLE content_articles ADD CONSTRAINT fk_content_articles_author FOREIGN KEY (author_id) REFERENCES auth.users(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_articles_category') THEN
        ALTER TABLE content_articles ADD CONSTRAINT fk_content_articles_category FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_articles_subcategory') THEN
        ALTER TABLE content_articles ADD CONSTRAINT fk_content_articles_subcategory FOREIGN KEY (subcategory_id) REFERENCES subcategories(id) ON DELETE SET NULL;
    END IF;
    
    -- content_exemples
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_exemples_author') THEN
        ALTER TABLE content_exemples ADD CONSTRAINT fk_content_exemples_author FOREIGN KEY (author_id) REFERENCES auth.users(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_exemples_category') THEN
        ALTER TABLE content_exemples ADD CONSTRAINT fk_content_exemples_category FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_exemples_subcategory') THEN
        ALTER TABLE content_exemples ADD CONSTRAINT fk_content_exemples_subcategory FOREIGN KEY (subcategory_id) REFERENCES subcategories(id) ON DELETE SET NULL;
    END IF;
    
    -- content_idees
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_idees_author') THEN
        ALTER TABLE content_idees ADD CONSTRAINT fk_content_idees_author FOREIGN KEY (author_id) REFERENCES auth.users(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_idees_category') THEN
        ALTER TABLE content_idees ADD CONSTRAINT fk_content_idees_category FOREIGN KEY (category_id) REFERENCES categories(id) ON DELETE SET NULL;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'fk_content_idees_subcategory') THEN
        ALTER TABLE content_idees ADD CONSTRAINT fk_content_idees_subcategory FOREIGN KEY (subcategory_id) REFERENCES subcategories(id) ON DELETE SET NULL;
    END IF;
END $$;
...
DROP TRIGGER IF EXISTS trigger_update_content_blogs_updated_at ON content_blogs;
CREATE TRIGGER trigger_update_content_blogs_updated_at
    BEFORE UPDATE ON content_blogs
    FOR EACH ROW
    EXECUTE FUNCTION update_content_updated_at();

DROP TRIGGER IF EXISTS trigger_update_content_articles_updated_at ON content_articles;
CREATE TRIGGER trigger_update_content_articles_updated_at
    BEFORE UPDATE ON content_articles
    FOR EACH ROW
    EXECUTE FUNCTION update_content_updated_at();

DROP TRIGGER IF EXISTS trigger_update_content_exemples_updated_at ON content_exemples;
CREATE TRIGGER trigger_update_content_exemples_updated_at
    BEFORE UPDATE ON content_exemples
    FOR EACH ROW
    EXECUTE FUNCTION update_content_updated_at();

DROP TRIGGER IF EXISTS trigger_update_content_idees_updated_at ON content_idees;
