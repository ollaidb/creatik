-- Script complet pour corriger toutes les tables et ajouter le contenu
...
-- 2. Ajouter les colonnes category et subcategory à exemplary_accounts
DO $$
BEGIN
...
-- 3. Créer la table sources si elle n'existe pas
...
-- 5. Activer RLS pour sources
ALTER TABLE sources ENABLE ROW LEVEL SECURITY;

-- 6. Politiques RLS pour sources
...
-- 9. Mettre à jour la fonction process_user_publications
...
-- 10. Supprimer les anciennes données pour éviter les doublons
DELETE FROM exemplary_accounts WHERE category = 'Activisme' AND subcategory = 'Campagnes';
DELETE FROM sources WHERE category = 'Activisme' AND subcategory = 'Campagnes';

-- 11. Ajouter des comptes de test dans exemplary_accounts pour Activisme/Campagnes
INSERT INTO exemplary_accounts (
  id,
  subcategory_id,
  account_name,
...
-- 12. Ajouter des sources de test dans sources pour Activisme/Campagnes
INSERT INTO sources (
  id,
  title,
  url,
...
-- 13. Vérifier que tout a été créé et ajouté
...
SELECT 'Colonnes ajoutées à exemplary_accounts:' as info;
SELECT column_name, data_type 
FROM information_schema.columns 
WHERE table_name = 'exemplary_accounts' 
