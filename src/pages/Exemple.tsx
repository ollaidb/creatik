...
const Exemple = () => {
  ...
  const selectedNetwork = searchParams.get('network') || 'twitter';
  ...
  // TODO: Créer useExemples hook
  const exemples = []; // Placeholder pour les exemples
  const exemplesLoading = false;
  ...
  const isLoading = isLevel2 ? 
    (subcategoryLevel2Loading || exemplesLoading) : 
    (subcategoryLoading || exemplesLoading);
  ...
  // Filtrer les exemples par searchTerm
  const filteredExemples = exemples.filter(exemple => 
    exemple.title.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleCopyExemple = async (exemple: string) => {
    try {
      await navigator.clipboard.writeText(exemple);
      toast({
        title: "Exemple copié"
      });
    } catch (err) {
  ...
  const handleAddToChallenge = (exempleId: string) => {
    // Logique pour ajouter à un challenge
    console.log('Adding to challenge:', exempleId);
  };

  const [activeTab, setActiveTab] = useState<'exemples' | 'comptes' | 'sources' | 'hashtags'>('exemples');

  const handleTabChange = (newTab: 'exemples' | 'comptes' | 'sources' | 'hashtags') => {
    setActiveTab(newTab);
  };
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200 dark:border-gray-700 px-4 py-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-semibold text-gray-900 dark:text-white truncate">
              {currentSubcategory?.name || 'Exemples'}
            </h1>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {filteredExemples.length} exemples disponibles
            </p>
      ...
      <div className="px-4 py-4">
        ...
        <div className="mb-6">
          <div className="max-w-lg mx-auto md:max-w-2xl">
            <LocalSearchBar 
              ...
              placeholder="Rechercher tous les exemples..."
              className="w-full"
            />
          </div>
        </div>
            ...
            <div className="flex gap-2 pb-2 min-w-max">
              {[
                ...
                return (
                  <motion.button
                    key={tab.key}
                    onClick={() => handleTabChange(tab.key as 'exemples' | 'comptes' | 'sources' | 'hashtags')}
                    className={`
        ...
        <div className="mt-6">
              ...
              {filteredExemples.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 dark:text-gray-500">
                    Les exemples incluent les tweets et contenus Twitter.
                  </p>
                </div>
              ) : (
                filteredExemples.map((exemple, index) => (
                  <motion.div key={`exemple-${exemple.id}`} variants={itemVariants}>
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleAddToChallenge(exemple.id)}
          ...
          {activeTab === 'comptes' && (
            <div className="text-center py-12">
              <p className="text-gray-500 dark:text-gray-400">
                Comptes disponibles pour {getNetworkDisplayName(detectedNetwork)}
              </p>
            </div>
          )}
          ...
          {activeTab === 'hashtags' && (
            <div className="text-center py-12">
              <p className="text-gray-500 dark:text-gray-400">
                Hashtags disponibles pour {getNetworkDisplayName(detectedNetwork)}
              </p>
            </div>
          )}
        </div>
