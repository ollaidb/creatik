...
import { ArrowLeft, Heart, Target, Plus, User, Clock, Star, Calendar, ChevronDown, ChevronUp, MessageCircle } from 'lucide-react';
...
const CommunityContent = () => {
  ...
  // Filtrer les challenges de type "content" et par réseau social
  const contentChallenges = challenges.filter(challenge => {
    const matchesType = challenge.challenge_type === 'content' || !challenge.challenge_type;
  ...
  const handleNetworkChange = (networkId: string) => {
    ...
    navigate(`/community/content?${params.toString()}`);
  };
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-muted-foreground">
              {contentChallenges.length} contenu{contentChallenges.length > 1 ? 's' : ''}
            </p>
          </div>
          <Button 
            size="sm"
      ...
      <div className="px-4 py-4">
        ...
        {contentChallenges.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-gray-500 dark:text-gray-400 mb-4">
              {searchTerm 
                ...
                : selectedNetwork !== 'all'
                ? `Aucun contenu disponible pour ${getNetworkDisplayName(selectedNetwork)}`
          ...
          <motion.div 
            ...
            animate="visible"
          >
            {contentChallenges.map((challenge) => renderChallengeCard(challenge))}
          </motion.div>
        )}
      </div>
      <Navigation />
