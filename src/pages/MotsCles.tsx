...
const MotsCles = () => {
  ...
  // TODO: Créer useMotsCles hook
  const motsCles = []; // Placeholder pour les mots-clés
  const motsClesLoading = false;
  ...
  const isLoading = isLevel2 ? 
    (subcategoryLevel2Loading || motsClesLoading) : 
    (subcategoryLoading || motsClesLoading);
  ...
  // Filtrer les mots-clés par searchTerm
  const filteredMotsCles = motsCles.filter(motCle => 
    motCle.title.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const getNetworkDisplayName = (networkId: string) => {
    switch (networkId) {
      ...
      default: return 'Mots-clés';
    }
  };

  const handleCopyMotCle = async (motCle: string) => {
    try {
      await navigator.clipboard.writeText(motCle);
      toast({
        title: "Mot-clé copié"
      });
    } catch (err) {
  ...
  const handleLikeMotCle = async (motCleId: string) => {
    try {
      await toggleMotsClesFavorite(motCleId);
      toast({
        title: isMotsClesFavorite(motCleId) ? "Retiré" : "Ajouté"
      });
    } catch (error) {
  ...
  const handleAddToChallenge = (motCleId: string) => {
    // Logique pour ajouter à un challenge
    console.log('Adding to challenge:', motCleId);
  };

  const [activeTab, setActiveTab] = useState<'mots-cles' | 'sources'>('mots-cles');

  const handleTabChange = (newTab: 'mots-cles' | 'sources') => {
    setActiveTab(newTab);
  };
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200 dark:border-gray-700 px-4 py-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-semibold text-gray-900 dark:text-white truncate">
              {currentSubcategory?.name || 'Mots-clés'}
            </h1>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {filteredMotsCles.length} mots-clés disponibles
            </p>
        ...
        <div className="mb-6">
          <div className="max-w-lg mx-auto md:max-w-2xl">
            <LocalSearchBar 
              ...
              placeholder="Rechercher tous les mots-clés..."
              className="w-full"
            />
          </div>
        </div>
            ...
            <div className="flex gap-2 pb-2 min-w-max">
              {[
                ...
                return (
                  <motion.button
                    key={tab.key}
                    onClick={() => handleTabChange(tab.key as 'mots-cles' | 'sources')}
                    className={`
              ...
              {filteredMotsCles.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 dark:text-gray-500">
                    Les mots-clés incluent les termes de recherche optimisés.
                  </p>
                </div>
              ) : (
                filteredMotsCles.map((motCle, index) => (
                  <motion.div key={`mot-cle-${motCle.id}`} variants={itemVariants}>
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleAddToChallenge(motCle.id)}
                          ...
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleLikeMotCle(motCle.id)}
                            className={`p-2 h-8 w-8 transition-all duration-200 ${
                              isMotsClesFavorite(motCle.id) 
