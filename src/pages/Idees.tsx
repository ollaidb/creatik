...
const Idees = () => {
  ...
  const selectedNetwork = searchParams.get('network') || 'instagram';
  ...
  // TODO: Créer useIdees hook
  const idees = []; // Placeholder pour les idées
  const ideesLoading = false;
  ...
  const isLoading = isLevel2 ? 
    (subcategoryLevel2Loading || ideesLoading) : 
    (subcategoryLoading || ideesLoading);
  ...
  // Filtrer les idées par searchTerm
  const filteredIdees = idees.filter(idee => 
    idee.title.toLowerCase().includes(searchTerm.toLowerCase())
  );
  ...
  const handleCopyIdee = async (idee: string) => {
    try {
      await navigator.clipboard.writeText(idee);
      toast({
        title: "Idée copiée"
      });
    } catch (err) {
  ...
  const handleAddToChallenge = (ideeId: string) => {
    // Logique pour ajouter à un challenge
    console.log('Adding to challenge:', ideeId);
  };

  const [activeTab, setActiveTab] = useState<'idees' | 'comptes' | 'sources' | 'hashtags'>('idees');

  const handleTabChange = (newTab: 'idees' | 'comptes' | 'sources' | 'hashtags') => {
    setActiveTab(newTab);
  };
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="sticky top-0 z-50 bg-white border-b border-gray-200 dark:border-gray-700 px-4 py-3"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <h1 className="text-lg font-semibold text-gray-900 dark:text-white truncate">
              {currentSubcategory?.name || 'Idées'}
            </h1>
            <p className="text-sm text-gray-600 dark:text-gray-400">
              {filteredIdees.length} idées disponibles
            </p>
        ...
        <div className="mb-6">
          <div className="max-w-lg mx-auto md:max-w-2xl">
            <LocalSearchBar 
              ...
              placeholder="Rechercher toutes les idées..."
              className="w-full"
            />
          </div>
        </div>
            ...
            <div className="flex gap-2 pb-2 min-w-max">
              {[
                ...
                return (
                  <motion.button
                    key={tab.key}
                    onClick={() => handleTabChange(tab.key as 'idees' | 'comptes' | 'sources' | 'hashtags')}
                    className={`
              ...
              {filteredIdees.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 dark:text-gray-500">
                    Les idées incluent les concepts créatifs pour Instagram.
                  </p>
                </div>
              ) : (
                filteredIdees.map((idee, index) => (
                  <motion.div key={`idee-${idee.id}`} variants={itemVariants}>
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleAddToChallenge(idee.id)}
