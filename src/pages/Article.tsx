...
const Article = () => {
  ...
  const selectedNetwork = searchParams.get('network') || 'article';
  ...
  // TODO: Créer useArticles hook
  const articles = []; // Placeholder pour les articles
  ...
  const isLoading = isLevel2 ? 
    (subcategoryLevel2Loading || articlesLoading) : 
    (subcategoryLoading || articlesLoading);
  ...
  // Filtrer les articles par searchTerm
  ...
  const handleCopyArticle = async (article: string) => {
    try {
  ...
  const handleAddToChallenge = (articleId: string) => {
    // Logique pour ajouter à un challenge
    console.log('Adding to challenge:', articleId);
  };

  const [activeTab, setActiveTab] = useState<'articles' | 'sources' | 'mots-cles'>('articles');

  const handleTabChange = (newTab: 'articles' | 'sources' | 'mots-cles') => {
    setActiveTab(newTab);
  };
            ...
            <div className="flex gap-2 pb-2 min-w-max">
              {[
                ...
                return (
                  <motion.button
                    key={tab.key}
                    onClick={() => handleTabChange(tab.key as 'articles' | 'sources' | 'mots-cles')}
                    className={`
              ...
              {filteredArticles.length === 0 ? (
                <div className="text-center py-12">
                  ...
                  <p className="text-sm text-gray-400 dark:text-gray-500">
                    Les articles incluent les articles de presse publiés.
                  </p>
                </div>
              ) : (
                filteredArticles.map((article, index) => (
                  <motion.div key={`article-${article.id}`} variants={itemVariants}>
                      ...
                      <div className="flex items-start justify-between">
                        ...
                        <div className="flex items-center gap-2 ml-4">
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => handleAddToChallenge(article.id)}
