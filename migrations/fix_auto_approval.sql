...
-- 11. Traiter les publications existantes qui sont déjà approuvées
-- Insérer automatiquement le contenu des publications déjà approuvées
INSERT INTO categories (name, description, color)
SELECT DISTINCT title, 'Catégorie approuvée', '#3B82F6'
FROM user_publications 
WHERE content_type = 'category' AND status = 'approved'
ON CONFLICT (name) DO NOTHING;

INSERT INTO subcategories (name, description, category_id)
SELECT DISTINCT up.title, 'Sous-catégorie approuvée', up.category_id
FROM user_publications up
WHERE up.content_type = 'subcategory' AND up.status = 'approved'
ON CONFLICT (name, category_id) DO NOTHING;

INSERT INTO content_titles (title, subcategory_id, type, platform)
SELECT DISTINCT up.title, up.subcategory_id, 'title', 'all'
FROM user_publications up
WHERE up.content_type = 'title' AND up.status = 'approved'
ON CONFLICT (title, subcategory_id) DO NOTHING; 